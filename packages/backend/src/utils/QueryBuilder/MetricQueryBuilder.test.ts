...
// A metric YAML relative-date filter (inThePast/inTheNext/...) is baked into
// compiled SQL at explore compile time. The query builder must re-evaluate that
// boundary against "now" at query time so the window rolls forward without a
// recompile.
describe('relative date metric filters evaluate at query time', () => {
    const COMPILE_TIME = new Date('2026-05-04T00:00:00Z').getTime();
    const QUERY_TIME = new Date('2026-06-04T00:00:00Z').getTime();

    const createdAt = EXPLORE_WITH_DATE_DIMENSION.tables.orders.dimensions
        .created_at as CompiledDimension;
    ...
    const relativeDateMetricQuery: CompiledMetricQuery = {
        exploreName: 'orders',
        dimensions: ['orders_created_at'],
        metrics: ['orders_order_count'],
        filters: {},
    ...
    test('swap still matches and rolls forward under a non-UTC query timezone', () => {
        // Guards against a non-UTC offset making the anchor substring silently
        // miss. (A DATE dimension's boundaries are timezone-independent, so the
        // fresh dates match the UTC case.)
        const rule = relativeRule('rdf1', FilterOperator.IN_THE_PAST);
        const baked = bakePredicateAt(rule, COMPILE_TIME);
        const explore = exploreWithMetric({
            ...baseMetric,
